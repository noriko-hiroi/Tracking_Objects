...
from config_trCOMITformer import Config
...
if TB_LOG:
    ...
    tb = SummaryWriter(log_dir=cf.tensorboard_log_dir)
...
if __name__ == "__main__":
    ...
    # model = models.Informer(cf, partition_model=None)
    ...
    with torch.no_grad():
        for it, (seqs, masks, seqlens, mmsis, time_starts) in pbar:
            ...
            for i_sample in range(cf.n_samples):
                ...
                input_coords = (inputs * v_ranges + v_roi_min)
                pred_coords = (preds * v_ranges + v_roi_min)
                d = torch.norm(pred_coords - input_coords, dim=-1) * masks
                # d = utils.haversine(input_coords, pred_coords) * masks
                error_ens[:, :, i_sample] = d[:, cf.init_seqlen:]
                l_pred_coords.append(pred_coords)
    ...
    pred_coords_concatenated = torch.cat(l_pred_coords, dim=0)
    pred_coords_reshaped = pred_coords_concatenated.view(-1, pred_coords_concatenated.size(-1)).cpu().numpy()
    np.savetxt(cf.savedir + 'pred_coords.csv', pred_coords_reshaped, delimiter=',')
    # l_pred_coords_concatenated = np.concatenate([tensor.cpu().numpy() for tensor in l_pred_coords], axis=0)
    # np.savetxt('l_pred_coords.csv', l_pred_coords_concatenated, delimiter=',')

    error_ens_np = error_ens.cpu().numpy()
    error_ens_reshaped = error_ens.view(-1, error_ens.size(-1)).cpu().numpy()
    np.savetxt(cf.savedir + 'error_ens.csv', error_ens_reshaped, delimiter=',')

    min_errors_np = min_errors.cpu().numpy()
    np.savetxt(cf.savedir + 'min_errors.csv', min_errors_np, delimiter=',')
    ...
    '''
    timestep = 90
    plt.plot(15, pred_errors[timestep], "o")
    plt.plot([15, 15], [0, pred_errors[timestep]], "r")
    plt.plot([0, 15], [pred_errors[timestep], pred_errors[timestep]], "r")
    plt.text(15.12, pred_errors[timestep] - 0.5, "{:.4f}".format(pred_errors[timestep]), fontsize=10)

    
    timestep = 120
    plt.plot(20, pred_errors[timestep], "o")
    plt.plot([20, 20], [0, pred_errors[timestep]], "r")
    plt.plot([0, 20], [pred_errors[timestep], pred_errors[timestep]], "r")
    plt.text(20.12, pred_errors[timestep] - 0.5, "{:.4f}".format(pred_errors[timestep]), fontsize=10)
    '''
